...
class ReflectionsPass extends postprocessing.Pass {
  constructor(ssrEffect, options = {}) {
    ...
    const width =
      options.width || typeof window !== "undefined" ? window.innerWidth : 2e3;
    const height =
    ...
    if (this.USE_MRT) {
      this.gBuffersRenderTarget = new THREE.WebGLMultipleRenderTargets(
        width,
        height,
        2,
        {
      ...
      this.webgl1DepthPass = new postprocessing.DepthPass(
        this._scene,
        this._camera
      );
...
class TemporalResolveMaterial extends THREE.ShaderMaterial {
  constructor(customComposeShader) {
    const fragmentShader2 = temporalResolve.replace(
      "#include <custom_compose_shader>",
      customComposeShader
    );
    super({
...
class VelocityPass extends postprocessing.Pass {
  ...
  setVelocityMaterialInScene() {
    ...
    for (const c of this.visibleMeshes) {
      ...
      if (
        ...
        originalMaterial.map instanceof THREE.VideoTexture
      ) {
  ...
  saveBoneTexture(object) {
    ...
    if (
      ...
      boneTexture = new THREE.DataTexture(
        boneMatrices,
        size,
        size,
        THREE.RGBAFormat,
...
class TemporalResolvePass extends postprocessing.Pass {
  ...
  setupFramebuffers(width, height) {
    ...
    this.accumulatedTexture = new THREE.FramebufferTexture(
      width,
      height,
      THREE.RGBAFormat
    );
      ...
      this._scene.traverse((c) => {
        if (!envMap && c.material && !c.material.envMap) {
          ...
          if (
            ...
            properties.envMap instanceof THREE.Texture
          )
